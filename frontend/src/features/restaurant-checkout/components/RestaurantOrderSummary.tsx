...
/**
 * Interactive leaf: order summary panel built from the shared client cart. Lists
 * each line item with quantity + line total, then subtotal / total. Sticky on large
 * screens so it stays in view while the form scrolls.
 */
  ...
  return (
    <Card className="h-fit lg:sticky lg:top-20">
      <CardContent className="space-y-4 p-6">
        ...
        <div className="flex items-center justify-between text-sm">
          <span className="text-muted-foreground">{t("summary.fees")}</span>
          <span className="text-muted-foreground">{t("summary.feesNote")}</span>
        </div>
